...
namespace Swordbot.Modules.BaseStates
{
    public abstract class BaseMeleeAttack : BaseSkillState, SteppedSkillDef.IStepSetter
    {
        ...
        protected PhysForceFlags physFlags = PhysForceFlags.None;
        ...
        public override void OnEnter()
        {
            ...
            characterDirection.forward =characterBody.inputBank.aimDirection;
            ...
            attack.physForceFlags = physFlags;
        ...
        private void FireAttack()
        {
            if (isAuthority)
            {
                List<HurtBox> l = new List<HurtBox>();
                if (attack.Fire(l))
                {
                    ModifyHitHurtBoxes(l);
                    OnHitEnemyAuthority();
                }
            }
        }

        protected virtual void ModifyHitHurtBoxes(List<HurtBox> hurtBoxes) {

            foreach (HurtBox box in hurtBoxes)
                SwordbotStaticComponent.Mark(box.transform);
        }
