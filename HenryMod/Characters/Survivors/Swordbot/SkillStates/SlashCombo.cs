...
namespace Swordbot.Survivors.Swordbot.SkillStates
{
    public class SlashCombo : BaseMeleeAttack
    {
        ...
        {
            footstepHandler = GetModelTransform()?.GetComponent<FootstepHandler>();
            ...
            isAir = !characterMotor.isGrounded;
            ...
            var soundPrefix = (isAir) ? "air_" : "";
            swingSoundString = $"Play_{soundPrefix}swing{swingIndex}";//"HenrySwordSwing"
            hitSoundString = $"Play_{soundPrefix}hit{swingIndex}";
        ...
        {
            Debug.Log("bro is swinging");
            var swingEffect = isAir ? SwordbotAssets.swordSwingAirEffect : SwordbotAssets.swordSwingEffect;

            Quaternion rotation = Quaternion.LookRotation(characterDirection.forward);
            Vector3 offset = (isAir) ? -Vector3.up + characterDirection.forward : 2 * (characterDirection.forward) - Vector3.up;

            if(swingIndex == 2)
            {
                swingEffect = SwordbotAssets.pokeEffect;
                rotation = Quaternion.LookRotation(Vector3.up, -characterDirection.forward);
                offset = characterDirection.forward * 4;
            }

            EffectManager.SimpleMuzzleFlash(swingEffect, gameObject, (swingIndex == 2 ? "pokeMuzzle" : "swordMuzzle"), true);
        }
